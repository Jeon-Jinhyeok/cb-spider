...
func CreateVPC(connectionName string, rsType string, reqInfo cres.VPCReqInfo, IDTransformMode string) (*cres.VPCInfo, error) {
	...
	if drv.GetDriverCapability().SINGLE_VPC {
		...
		if len(vpcIIDInfoList) > 0 {
			err := fmt.Errorf("%s-%s can have only 1 VPC, but already have a VPC %s", rsType, connectionName, vpcIIDInfoList[0].NameId)
			cblog.Error(err)
			return nil, err
		}
	}
	...
	for _, subnetInfo := range info.SubnetInfoList {
		...
		if err != nil {
			...
			if err3 != nil {
				cblog.Error(err3)
				return nil, fmt.Errorf("%s, %s", err, err3)
			}
			// (3) for Subnet IID
...
func AddSubnet(connectionName string, rsType string, vpcName string, reqInfo cres.SubnetInfo, IDTransformMode string) (*cres.VPCInfo, error) {
	...
	if bool_ret {
		err := fmt.Errorf("%s-%s already exists!", rsType, reqInfo.IId.NameId)
		cblog.Error(err)
		return nil, err
	}
	// (2) create Resource
...
func RemoveSubnet(connectionName string, vpcName string, nameID string, force string) (bool, error) {
	...
	result, err = handler.RemoveSubnet(getDriverIID(cres.IID{NameId: iidVPCInfo.NameId, SystemId: iidVPCInfo.SystemId}), driverIId)
	if err != nil {
		cblog.Error(err)
		if force != "true" {
			return false, err
...
func RemoveCSPSubnet(connectionName string, vpcName string, systemID string) (bool, error) {
	...
	result, err = handler.RemoveSubnet(getDriverIID(cres.IID{NameId: iidVPCInfo.NameId, SystemId: iidVPCInfo.SystemId}), iid)
	if err != nil {
		cblog.Error(err)
		return false, err
	}
...
func DeleteVPC(connectionName string, rsType string, nameID string, force string) (bool, error) {
	...
	driverIId := getDriverIID(cres.IID{NameId: iidInfo.NameId, SystemId: iidInfo.SystemId})
	result := false
	result, err = handler.DeleteVPC(driverIId)
	if err != nil {
		cblog.Error(err)
		if force != "true" {
			return false, err
